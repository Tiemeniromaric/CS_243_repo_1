...
const { registerSchema, loginSchema, projectSchema, timeEntrySchema } = require('./models');
...
const authLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 20, // limit each IP to 20 auth attempts per windowMs (increased for development)
...
app.post('/projects', authenticate, (req, res) => {
  ...
  const { name } = req.body;
  db.query('INSERT INTO projects (user_id, name) VALUES (?, ?)', [req.user.id, name], (err) => {
...
app.put('/projects/:id', authenticate, (req, res) => {
  ...
  const { name } = req.body;
  db.query('UPDATE projects SET name = ? WHERE id = ? AND user_id = ?', [name, req.params.id, req.user.id], (err) => {
...
app.delete('/projects/:id', authenticate, (req, res) => {
  ...
  db.query('DELETE FROM time_entries WHERE project_id = ? AND user_id = ?', [req.params.id, req.user.id], (err) => {
    if (err) return res.status(500).json({ error: 'Failed to delete associated time entries' });
    // Then delete the project
    db.query('DELETE FROM projects WHERE id = ? AND user_id = ?', [req.params.id, req.user.id], (err) => {
...
app.post('/time', authenticate, (req, res) => {
  ...
  db.query('INSERT INTO time_entries (user_id, project_id, start_time, end_time, duration, notes) VALUES (?, ?, ?, ?, ?, ?)', 
    [req.user.id, project_id, start_time, end_time, duration, notes], (err) => {
...
app.get('/time', authenticate, (req, res) => {
  ...
  let query = 'SELECT te.*, p.name AS project_name FROM time_entries te JOIN projects p ON te.project_id = p.id WHERE te.user_id = ?';
  const params = [req.user.id];
  if (project_id) { query += ' AND te.project_id = ?'; params.push(project_id); }
  if (date) { query += ' AND DATE(te.start_time) = ?'; params.push(date); }
  db.query(query, params, (err, results) => {
    if (err) return res.status(500).json({ error: 'Failed to fetch time entries' });
    res.json(results);
  });
});
